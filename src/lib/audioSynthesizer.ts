...
class HudAudioSynthesizer {
  ...
  // Synthesize realistic acoustic cardiac Lub-Dub sound
  public playLubDubSound(volume: number = 0.15) {
    const ctx = this.getContext();
    ...
    try {
      ...
      // "Lub" (S1 - Atrial contraction / AV valves closing) - Deep 70Hz -> 30Hz pulse
      const osc1 = ctx.createOscillator();
      ...
      // "Dub" (S2 - Ventricular contraction / Semilunar valves closing) - 100Hz -> 40Hz
      const osc2 = ctx.createOscillator();
      const gain2 = ctx.createGain();
      osc2.type = 'sine';
      osc2.frequency.setValueAtTime(100, now + 0.12);
      osc2.frequency.exponentialRampToValueAtTime(40, now + 0.20);
